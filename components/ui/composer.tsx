...
export const Composer: FC<ComposerProps> = ({
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (!composerRef.current?.contains(target) && !target.closest('.slash-command-dropdown')) {
        setIsToolsDropdownOpen(false);
        setIsContextMenuOpen(false);
      }
    };
  ...
  const canSubmit = currentValue.trim();
  // || attachedFiles.length > 0;
  ...
  return (
    <div className={cn('relative w-full', className)}>
      ...
      >
        ...
        {/* <FilePreview files={attachedFiles} onRemove={onRemoveFile} className="rounded-xl" /> */}
          ...
          <div className="flex items-center gap-1">
            ...
            {showToolsButton && (
              ...
              >
                ...
                <ToolCase className="text-zinc-500 dark:text-zinc-400" />
                {isToolsDropdownOpen && (
                  <span
